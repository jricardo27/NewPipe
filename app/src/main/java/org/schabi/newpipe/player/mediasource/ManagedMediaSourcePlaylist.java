...
public class ManagedMediaSourcePlaylist {
     ...
     * at the given index. If this index is out of bound, then the removal is
     * ignored.
     *
     ...
     * from the given source index to the target index. If either index is out of
     * bound,
     ...
     *                         after the media source has been removed from the
     *                         playlist
     */
    ...
    /**
     * Updates the {@link ManagedMediaSource} in {@link ConcatenatingMediaSource}
     * at the given index with a given {@link ManagedMediaSource}. If the index is
     * out of bound,
    ...
    public synchronized void update(final int index, @NonNull final ManagedMediaSource source,
        ...
        // Add and remove are sequential on the same thread, therefore here, the
        // exoplayer
        // message queue must receive and process add before remove, effectively
        // treating them
        // as atomic.

        // Since the finalizing action occurs strictly after the timeline has completed
        // all its changes on the playback thread, thus, it is possible, in the
        // meantime,
        // other calls that modifies the playlist media source occur in between. This
        // makes
        // it unsafe to call remove as the finalizing action of add.
        internalSource.addMediaSource(index + 1, source);

        // Because of the above race condition, it is thus only safe to synchronize the
        // player
        // in the finalizing action AFTER the removal is complete and the timeline has
        // changed.
